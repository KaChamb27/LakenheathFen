...
(function() {
    ...
    function createPopup(feature, id) {
        ...
        if (id === 'name') {
            return L.popup().setContent('<p style="font-size:12px"><b>Address: </b>' + prop.name + '</p>');
        } else {
            if (prop.type === null) {
    ...
    function get_features(features, id) {
        ...
        features.forEach(function(feature) {
            if (id === 'poi'||id === 'trails'||id === 'public_t') {
                query_set.add(feature.name);
            } else if (id === 'services') {
                    query_set.add(feature.type);
            } else {
                query_set.add(feature.cartodbid);
            }
        });
    ...
    function load_data(self) {
        $.getJSON(base_url + get_sql_query(self.id), function(data) {
            ...
            get_features(data.rows, self.id);
     ...
     */
    var base_url = 'https://knelson4.carto.com/api/v2/sql?q=';
